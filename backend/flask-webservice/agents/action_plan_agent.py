...
class ActionPlanAgent():
    ...
    def get_response(self,messages):
        ...
        body = {
            ...
            \"sentiment\":\"negative\" or \"positive\" This is the given sentiment .
    ...
    def process_response(self, response_json):
        ...
        message_match = re.search(r'"message"\s*:\s*"(.*?)"', generated_text)
        sentiment_match = re.search(r'"sentiment"\s*:\s*"(.*?)"', generated_text)
        action1_match = re.search(r'"action1"\s*:\s*"(.*?)"', generated_text)
        action2_match = re.search(r'"action2"\s*:\s*"(.*?)"', generated_text)
        
        extracted_data = {
            "message": message_match.group(1) if message_match else "NA",
            "sentiment": sentiment_match.group(1) if sentiment_match else "unknown",
            "action1": action1_match.group(1) if sentiment_match else "unknown",
            "action2": action2_match.group(1) if sentiment_match else "unknown"
        }
